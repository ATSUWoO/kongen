  ...
  <body>
    ...
    <script type="text/javascript">
	  ...
	  $.ajax({
		  ...
		  if(data[i].pre_eval_reason == null){
		      data[i].pre_eval_reason = "";
		  }
		  let eval_data = "なぜこの評価にしたのか";
		  if(data[i].pre_eval_value == null){
		      data[i].pre_eval_value = 3;
		  }
		  target.innerHTML += "<tr><td>" +
		                          ...
		                          "</td><td>" +
		                              "<input type='range' id='eval"+data[i].q_id+ "' min='1' max='5' value='"+ data[i].pre_eval_value +"' />" +
		                          "</td><td>" +
		    "<textarea id='reason"+data[i].q_id+"' rows='10' cols='50' required='required' placeholder='"+ eval_data +"'>"+ data[i].pre_eval_reason +"</textarea>" +
		                      "</td></tr>";
		  q_list.push(data[i]);
              }
